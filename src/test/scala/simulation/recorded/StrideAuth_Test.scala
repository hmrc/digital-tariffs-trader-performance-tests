...
//    .exec(http("Landing_Page")
//    .get("http://" + uri2 + ":9581/tariff-classification/")
//    .get("/tariff-classification")
//    .headers(headers_2),
//    //.check(status.is(303)),


    .exec(http("StrideAuth")
      .get("http://" + uri2 + ":9041/stride/sign-in?successURL=http%3A%2F%2Flocalhost%3A9581%2Ftariff-classification%2F&origin=tariff-classification-frontend")
      .headers(headers_7)
      .check(status.is(200))
      .check(saveSamlRequest())
      .check(saveRelayState()))
    .exec(http("StrideAuth2")
      .get("http://" + uri2 + ":9043/stride-idp-stub/auth-request?SAMLRequest=${samlRequest}RelayState=${relayState}")
      .headers(headers_7))
    ...
    .resources(http("request_12")
      .get("http://" + uri2 + ":9043/stride-idp-stub/redirect-to-stride?redirectUrl=http%3A%2F%2Flocalhost%3A9041%2Fstride%2Fauth-response&encodedSamlResponse=${samlResponse}relayState=${relayState}")
      .headers(headers_7),
      http("request_13")
        .post("http://" + uri2 + ":9041/stride/auth-response")
        .headers(headers_11)
